...
for epoch in range(num_epochs):
    ...
    for state, action in zip(X_train.values, y_train.values):
        ...
        if (counter + 1)%200 ==0:
            ...
            predicted_diameter.append(state[1]+reward)
            batch_mse = mean_squared_error(setpoint, predicted_diameter)
            mse_history.append(batch_mse)
            counter_history.append(counter+1)
            print(counter, "Setpoint ", state[0], "predicted diameter ", predicted_diameter[0], "error", error, "reward ", reward, "epoch ", epoch)
...
plt.plot(counter_history, setpoint, label='Setpoint', marker='o')  # Plot actual RPM
plt.plot(counter_history, predicted_diameter, label='Predicted Diameter', marker='x')  # Plot predicted RPM

plt.title('Setpoint and Predicted Diameter over Time')  # Set the title of the graph
plt.xlabel('Counter')  # Set the x-axis label
plt.ylabel('Diameter')  # Set the y-axis label
...
#next_states = np.full(y_test.shape, next_states)
score_history.append(total_reward)
print(f'Epoch {epoch+1}/{num_epochs}, Total Reward: {total_reward}')
print ("Mean Squared Error: ", batch_mse)
